...
const { loading, pageCount, pageSize, data, page } = usePagination(
  (page) =>
    getCategoryFilterListApi({
      page,
      category: [category.value, subCategory.value].filter(Boolean).join('_'),
      order: order.value,
    }),
  {
    initialPage: routePage.value,
    initialPageSize: 80,
    watchingStates: [order, category, subCategory],
  ...
  <v-card>
    <v-card-text>
      <v-data-iterator :items="data" :items-per-page="pageSize" :loading="loading">
        ...
        <template #no-data>
          <v-empty-state title="来到了一个无人区" :image="EMPTY_STATE_IMG"></v-empty-state>
        </template>

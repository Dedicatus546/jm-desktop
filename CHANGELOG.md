...
- 更新了 jm 的图片解密逻辑，修复由于 jm 解密逻辑变更造成的部分 gif 图片被解密问题

# 2.1.2

- 修复升级 vuetify 版本后签到页面 ui 错误问题。
- 修复个人中心历史评论页面、本子分类页面、快速搜索页面在路由切换时没有恢复到上次的页码问题。
- 修复更新 vuetify 后找不到 v-calendar 组件问题。
- 去除打开软件时的全局 loading ，改为在拦截器中初始化所需数据。现在第一次使用软件时，如果未设置代理可以直接打开设置页面设置，然后重启即可。
- 修复登录页 v-textField 组件颜色不正确问题。
- 修复由于 vuetify4 使用 @layer 导致 unocss 样式层级过高造成的非正确覆盖问题。
- 修复主进程的 express 服务的路由回退到 index.html 所设置的路径不正确问题。
- 更新依赖。
